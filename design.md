...
### Collapsing Strategy
- Top nav collapses entirely on mobile, replaced by a bottom `MobileBottomNav` component.
- Photography stays full-bleed at every breakpoint — never collapses to a margin'd container.
- Card grids convert to horizontal snap-scrolling carousels on mobile to save vertical space and provide a native app feel.
...
- **Bottom Navigation**: Primary mobile navigation is handled via a floating, glassmorphic bottom bar (`MobileBottomNav`) instead of a traditional top hamburger menu. This ensures thumb-reachability and mimics native apps.
- **Glassmorphism & Neon**: Heavy use of `backdrop-blur`, subtle white borders (`rgba(255,255,255,0.1)`), and neon glowing accents (`drop-shadow` in red/grey) to create a futuristic cyberpunk/industrial aesthetic.
- **Horizontal Carousels**: Lists of cards (Services, Portfolio, Testimonials, Reasons) become horizontal carousels (`overflow-x-auto snap-x`) on mobile.
- **Safe Areas**: Use `env(safe-area-inset-bottom)` to pad the bottom of the page and fixed elements so they are not obscured by iOS/Android home indicators.
- **Touch Ergonomics**: All interactive elements (buttons, links) are given a minimum height of 48px on mobile to ensure they are easily tappable.
